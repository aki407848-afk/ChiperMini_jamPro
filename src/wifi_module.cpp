...
// FIXED: Proper variable declarations in rotate_mac_if_needed()
...
void rotate_mac_if_needed() {
    // ✅ ОБЪЯВЛЯЕМ ПЕРЕМЕННЫЕ ПЕРЕД ИСПОЛЬЗОВАНИЕМ    bool timeExpired = (millis() - lastMacRotation >= MAC_ROTATION_INTERVAL_MS);
...
void wifi_deauth_start(const String& bssid_str, uint8_t channel) {
    ...
    // Устанавливаем стартовый MAC
    esp_wifi_set_promiscuous(false);
    delay(2);    esp_wifi_set_mac(WIFI_IF_STA, current_mac);
