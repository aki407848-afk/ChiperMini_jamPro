...
<div class="card">
<h3>⚡ Deauth Attack</h3>
<p class="status">Target: <span id="target">None</span></p><p class="status">Channel: <span id="channel">-</span></p>
...
async function scan() {
    ...
    data.networks.forEach((n, i) => {
        html += `<div class="network">
            <span>${n.ssid || '(hidden)'} <small style="color:#666">[${n.bssid}]</small></span>
...
async function refreshStatus() {
    ...
    document.getElementById('uptime').textContent = Math.floor(data.uptime / 1000) + 's';    document.getElementById('mac').textContent = data.mac;
...
// ======== JSON HELPERS ========
String getScanResultsJSON() {    String json = "{\"networks\":[";
