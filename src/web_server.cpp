...
// FIXED: All forward declarations for external functions
...
// ======== FORWARD DECLARATIONS: WIFI MODULE FUNCTIONS ========
...
<div class="container">
<h1>🛡️ ChiperMini_jamPro</h1><div class="card">
<h3>📡 WiFi Scan</h3>
...
async function api(ep){try{const r=await fetch(API+ep);return await r.json();}catch(e){log('API error: '+e);return null;}}
function log(msg){const el=document.getElementById('log');const t=new Date().toLocaleTimeString();el.innerHTML='['+t+'] '+msg+'<br>'+el.innerHTML;if(el.children.length>50)el.lastChild.remove();}
async function scan(){log('Scanning...');document.getElementById('scanResults').innerHTML='Scanning...';const d=await api('/scan');if(!d)return;let h='';d.networks.forEach((n,i)=>{h+='<div class="network"><span>'+(n.ssid||'(hidden)')+' <small style="color:#666">['+n.bssid+']</small></span><span><span class="channel-badge">CH'+n.channel+'</span> '+n.rssi+'dBm <button onclick="selectTarget(\''+n.bssid+'\','+n.channel+')" style="margin-left:5px">→</button></span></div>';});document.getElementById('scanResults').innerHTML=h;log('Found '+d.networks.length+' networks');}
function selectTarget(b,c){targetBSSID=b;targetChannel=c;document.getElementById('target').textContent=b;document.getElementById('channel').textContent=c;document.getElementById('startBtn').disabled=false;log('Target: '+b+' CH'+c);}
async function startAttack(){if(!targetBSSID)return;log('Starting...');const r=await api('/attack/start?bssid='+targetBSSID+'&channel='+targetChannel);if(r?.success){document.getElementById('attackStatus').textContent='ACTIVE';document.getElementById('attackStatus').style.color='#f44';document.getElementById('startBtn').disabled=true;document.getElementById('stopBtn').disabled=false;log('Attack started');}}
async function stopAttack(){log('Stopping...');const r=await api('/attack/stop');if(r?.success){document.getElementById('attackStatus').textContent='Idle';document.getElementById('attackStatus').style.color='#0f0';document.getElementById('startBtn').disabled=!targetBSSID;document.getElementById('stopBtn').disabled=true;log('Attack stopped');}}
async function refreshStatus(){const d=await api('/status');if(!d)return;document.getElementById('version').textContent=d.version;document.getElementById('uptime').textContent=Math.floor(d.uptime/1000)+'s';document.getElementById('mac').textContent=d.mac;document.getElementById('project').textContent=d.project;document.getElementById('attackStatus').textContent=d.attack_active?'ACTIVE':'Idle';document.getElementById('attackStatus').style.color=d.attack_active?'#f44':'#0f0';}
setInterval(refreshStatus,5000);refreshStatus();log('Loaded');
</script>
</body></html>
)rawliteral";
...
String getScanResultsJSON() {
    ...
    for (int i = 0; i < min(n, MAX_SCAN_RESULTS); i++) {
        ...
        json += "\"bssid\":\"" + WiFi.BSSIDstr(i) + "\",";        json += "\"rssi\":" + String(WiFi.RSSI(i)) + ",";
...
String getSystemStatusJSON() {
    ...
    json += "\"mac\":\"";
    for (int i = 0; i < 6; i++) { json += String(mac[i], HEX); if (i < 5) json += ":"; }
    json += "\",";
...
void setupWebServer() {
    ...
    server.on("/attack/start", HTTP_GET, [](AsyncWebServerRequest *request){
        if (request->hasParam("bssid") && request->hasParam("channel")) {
            ...
            wifi_deauth_start(bssid, channel);  // ✅ Теперь видит
